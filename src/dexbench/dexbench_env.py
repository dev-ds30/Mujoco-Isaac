...
ASSET=os.path.join(os.path.dirname(__file__),"..","..","assets","dexbench_two.xml")
...
class DexBenchEnv:
    ...
    @property
    def action_space(self): return 4
    @property
    def observation_space(self): return 18
    def _obs(self,a):
        qpos=self.data.qpos.copy(); xyz=qpos[:3]; wxyz=qpos[3:7]
        quat_xyzw=np.array([wxyz[1],wxyz[2],wxyz[3],wxyz[0]],np.float32)
        jids=[self.model.joint(n).id for n in ["a1","a2","b1","b2"]]
        q=np.array([self.data.qpos[i] for i in jids],np.float32)
        tfeat=np.array([self.t/self.episode_length,0,0],np.float32)
    ...
    def reset(self, seed=None):
        ...
        return self._obs(np.zeros(4,np.float32)), {}
